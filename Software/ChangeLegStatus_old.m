       ...
       | Neighbor ~= 0 % this means that we are not pressing anything, we just look for the next footprint  
        ...
        if x <= 0 | x >= picsize(2) | y <= 0 | y >= picsize(1)
            ...
            % Set data to specified
              ...
              text(v.MouseTrack.LegLF.Centroid{MouseIndex}(N,2)+17,v.MouseTrack.LegLF.Centroid{MouseIndex}(N,1)-17,'LF', 'Color', p.colorFront, 'LineWidth', 2, 'FontSize', p.FootFontSize); 
            ...
            % Place footprint where the user placed it for all the frames until 1 before the next placement of the same foot.
              ...
              while i ~= -1
                % exit if next footprint is already on otherwise go till the end of the set of frames
                  if length(handles.p.FileList) > i & i < v.MouseTrack.TrackLength{MouseIndex} 
                    ...
                    % if next footprint is not present, update 'i'th one with selection    
                        ...
                        % read image corresponding to frame
                          v.pic = PictureReader(i, p);  
                          % subtract background
                            v = FilterImage(v,handles.p);  
                        % Set data to specified
                          v.MouseTrack.LegLF.Centroid{MouseIndex}(i,2) = x;
                          v.MouseTrack.LegLF.Centroid{MouseIndex}(i,1) = y;
                        % Identify and save brightness - within handles.p.MaxFingerDistance
                        % calculate edges of foot
                          v.MouseTrack.LegLF.FootMinX{MouseIndex}(i) = max(y-handles.p.MaxFingerDistance,1);
                          v.MouseTrack.LegLF.FootMaxX{MouseIndex}(i) = min(y+handles.p.MaxFingerDistance,length(handles.v.pic.foot(:,1)));
                          v.MouseTrack.LegLF.FootMinY{MouseIndex}(i) = max(x-handles.p.MaxFingerDistance,1);
                          v.MouseTrack.LegLF.FootMaxY{MouseIndex}(i) = min(x+handles.p.MaxFingerDistance,length(handles.v.pic.foot(1,:)));    
                        % sized rectangle (for simplicity)
                          indX = round(v.MouseTrack.LegLF.FootMinX{MouseIndex}(i)) : round(v.MouseTrack.LegLF.FootMaxX{MouseIndex}(i));
                          indY = round(v.MouseTrack.LegLF.FootMinY{MouseIndex}(i)) : round(v.MouseTrack.LegLF.FootMaxY{MouseIndex}(i));
                          FootMatrix = v.pic.foot(indX,indY);
                          indFoot = find(FootMatrix(:) > 0);
                        % calculate total footsize
                          v.MouseTrack.LegLF.FootSize{MouseIndex}(i) = length(indFoot);
                        % calculate total brightness
                          v.MouseTrack.LegLF.FootTotalBrightness{MouseIndex}(i) = sum(sum(FootMatrix));
                        % calcuate maximum brightness of foot
                          v.MouseTrack.LegLF.FootMaxBrightness{MouseIndex}(i) = max(max(FootMatrix));                      
                        % increase i
                          i = i + 1;                          
                      end;
        ...
        % overplot existing leg, but only if it exists (it should as otherwise the togglebutton wouldn't have been down)
          if v.MouseTrack.LegLF.Centroid{MouseIndex}(N,1) > 0
            ...
            text(v.MouseTrack.LegLF.Centroid{MouseIndex}(N,2)+17,v.MouseTrack.LegLF.Centroid{MouseIndex}(N,1)-17,'LF', 'Color', p.colorFront/2, 'LineWidth', 2, 'FontSize', p.FootFontSize); 
          end
       ...
       | Neighbor ~= 0 % this means that we are not pressing anything, we just look for the next footprint  
        ...
        if x <= 0 | x >= picsize(2) | y <= 0 | y >= picsize(1)
            ...
            % Set data to specified
              ...
              text(v.MouseTrack.LegRF.Centroid{MouseIndex}(N,2)+17,v.MouseTrack.LegRF.Centroid{MouseIndex}(N,1)-17,'RF', 'Color', p.colorFront, 'LineWidth', 2, 'FontSize', p.FootFontSize); 
            % Identify and save brightness - within handles.p.MaxFingerDistance
            % calculate edges of foot
                ...
                % exit if next footprint is already on otherwise go till the end of the set of frames
                  if length(handles.p.FileList) > i & i < v.MouseTrack.TrackLength{MouseIndex} 
                    ...
                    % if next footprint is not present, update 'i'th one with selection    
                        ...
                        % Identify and save brightness - within handles.p.MaxFingerDistance
                        % calculate edges of foot
                          v.MouseTrack.LegRF.FootMinX{MouseIndex}(i) = max(y-handles.p.MaxFingerDistance,1);
                          v.MouseTrack.LegRF.FootMaxX{MouseIndex}(i) = min(y+handles.p.MaxFingerDistance,length(handles.v.pic.foot(:,1)));
                          v.MouseTrack.LegRF.FootMinY{MouseIndex}(i) = max(x-handles.p.MaxFingerDistance,1);
                          v.MouseTrack.LegRF.FootMaxY{MouseIndex}(i) = min(x+handles.p.MaxFingerDistance,length(handles.v.pic.foot(1,:)));    
                        % sized rectangle (for simplicity)
                          indX = round(v.MouseTrack.LegRF.FootMinX{MouseIndex}(i)) : round(v.MouseTrack.LegRF.FootMaxX{MouseIndex}(i));
                          indY = round(v.MouseTrack.LegRF.FootMinY{MouseIndex}(i)) : round(v.MouseTrack.LegRF.FootMaxY{MouseIndex}(i));
                          FootMatrix = v.pic.foot(indX,indY);
                          indFoot = find(FootMatrix(:) > 0);
                        % calculate total footsize
                          v.MouseTrack.LegRF.FootSize{MouseIndex}(i) = length(indFoot);
                        % calculate total brightness
                          v.MouseTrack.LegRF.FootTotalBrightness{MouseIndex}(i) = sum(sum(FootMatrix));
                        % calcuate maximum brightness of foot
                          v.MouseTrack.LegRF.FootMaxBrightness{MouseIndex}(i) = max(max(FootMatrix));                      
                        % increase i                          
                          i = i + 1;
                      end;
                % if there is no more data, set up current one and then quit      
        ...
        % overplot existing leg, but only if it exists (it should as otherwise the togglebutton wouldn't have been down)
          if v.MouseTrack.LegRF.Centroid{MouseIndex}(N,1) > 0
            ...
            text(v.MouseTrack.LegRF.Centroid{MouseIndex}(N,2)+17,v.MouseTrack.LegRF.Centroid{MouseIndex}(N,1)-17,'RF', 'Color', p.colorFront/2, 'LineWidth', 2, 'FontSize', p.FootFontSize); 
          end
        % Turn footprint off till as long as the current footprint is placed (i.e. until there is a gap).
          i = N;
        while i ~= -1
       ...
       | Neighbor ~= 0 % this means that we are not pressing anything, we just look for the next footprint  
        ...
        if x <= 0 | x >= picsize(2) | y <= 0 | y >= picsize(1)
            ...
            % Set data to specified
              ...
              text(v.MouseTrack.LegLH.Centroid{MouseIndex}(N,2)+17,v.MouseTrack.LegLH.Centroid{MouseIndex}(N,1)-17,'LH', 'Color', p.colorHind, 'LineWidth', 2, 'FontSize', p.FootFontSize); 
            % Identify and save brightness - within handles.p.MaxFingerDistance
            % calculate edges of foot
                ...
                % exit if next footprint is already on otherwise go till the end of the set of frames
                  if length(handles.p.FileList) > i & i < v.MouseTrack.TrackLength{MouseIndex} 
                    ...
                    % if next footprint is not present, update 'i'th one with selection    
                        ...
                        % Identify and save brightness - within handles.p.MaxFingerDistance
                        % calculate edges of foot
                          v.MouseTrack.LegLH.FootMinX{MouseIndex}(i) = max(y-handles.p.MaxFingerDistance,1);
                          v.MouseTrack.LegLH.FootMaxX{MouseIndex}(i) = min(y+handles.p.MaxFingerDistance,length(handles.v.pic.foot(:,1)));
                          v.MouseTrack.LegLH.FootMinY{MouseIndex}(i) = max(x-handles.p.MaxFingerDistance,1);
                          v.MouseTrack.LegLH.FootMaxY{MouseIndex}(i) = min(x+handles.p.MaxFingerDistance,length(handles.v.pic.foot(1,:)));    
                        % sized rectangle (for simplicity)
                          indX = round(v.MouseTrack.LegLH.FootMinX{MouseIndex}(i)) : round(v.MouseTrack.LegLH.FootMaxX{MouseIndex}(i));
                          indY = round(v.MouseTrack.LegLH.FootMinY{MouseIndex}(i)) : round(v.MouseTrack.LegLH.FootMaxY{MouseIndex}(i));
                          FootMatrix = v.pic.foot(indX,indY);
                          indFoot = find(FootMatrix(:) > 0);
                        % calculate total footsize
                          v.MouseTrack.LegLH.FootSize{MouseIndex}(i) = length(indFoot);
                        % calculate total brightness
                          v.MouseTrack.LegLH.FootTotalBrightness{MouseIndex}(i) = sum(sum(FootMatrix));
                        % calcuate maximum brightness of foot
                          v.MouseTrack.LegLH.FootMaxBrightness{MouseIndex}(i) = max(max(FootMatrix));                      
                        % increase i                          
                          i = i + 1;
                      end;
                % if there is no more data, set up current one and then quit      
        ...
        % overplot existing leg, but only if it exists (it should as otherwise the togglebutton wouldn't have been down)
          if v.MouseTrack.LegLH.Centroid{MouseIndex}(N,1) > 0
            ...
            text(v.MouseTrack.LegLH.Centroid{MouseIndex}(N,2)+17,v.MouseTrack.LegLH.Centroid{MouseIndex}(N,1)-17,'LH', 'Color', p.colorHind/2, 'LineWidth', 2, 'FontSize', p.FootFontSize); 
          end
        % Turn footprint off till as long as the current footprint is placed (i.e. until there is a gap).
          i = N;
        while i ~= -1
       ...
       | Neighbor ~= 0 % this means that we are not pressing anything, we just look for the next footprint  
        ...
        if x <= 0 | x >= picsize(2) | y <= 0 | y >= picsize(1)
            ...
            % Set data to specified
              ...
              text(v.MouseTrack.LegRH.Centroid{MouseIndex}(N,2)+17,v.MouseTrack.LegRH.Centroid{MouseIndex}(N,1)-17,'RH', 'Color', p.colorHind, 'LineWidth', 2, 'FontSize', p.FootFontSize); 
            % Identify and save brightness - within handles.p.MaxFingerDistance
            % calculate edges of foot
                ...
                % exit if next footprint is already on otherwise go till the end of the set of frames
                  if length(handles.p.FileList) > i & i < v.MouseTrack.TrackLength{MouseIndex} 
                    ...
                    % if next footprint is not present, update 'i'th one with selection    
                        ...
                        % Identify and save brightness - within handles.p.MaxFingerDistance
                        % calculate edges of foot
                          v.MouseTrack.LegRH.FootMinX{MouseIndex}(i) = max(y-handles.p.MaxFingerDistance,1);
                          v.MouseTrack.LegRH.FootMaxX{MouseIndex}(i) = min(y+handles.p.MaxFingerDistance,length(handles.v.pic.foot(:,1)));
                          v.MouseTrack.LegRH.FootMinY{MouseIndex}(i) = max(x-handles.p.MaxFingerDistance,1);
                          v.MouseTrack.LegRH.FootMaxY{MouseIndex}(i) = min(x+handles.p.MaxFingerDistance,length(handles.v.pic.foot(1,:)));    
                        % sized rectangle (for simplicity)
                          indX = round(v.MouseTrack.LegRH.FootMinX{MouseIndex}(i)) : round(v.MouseTrack.LegRH.FootMaxX{MouseIndex}(i));
                          indY = round(v.MouseTrack.LegRH.FootMinY{MouseIndex}(i)) : round(v.MouseTrack.LegRH.FootMaxY{MouseIndex}(i));
                          FootMatrix = v.pic.foot(indX,indY);
                          indFoot = find(FootMatrix(:) > 0);
                        % calculate total footsize
                          v.MouseTrack.LegRH.FootSize{MouseIndex}(i) = length(indFoot);
                        % calculate total brightness
                          v.MouseTrack.LegRH.FootTotalBrightness{MouseIndex}(i) = sum(sum(FootMatrix));
                        % calcuate maximum brightness of foot
                          v.MouseTrack.LegRH.FootMaxBrightness{MouseIndex}(i) = max(max(FootMatrix));                      
                        % increase i                          
                          i = i + 1;
                      end;
                % if there is no more data, set up current one and then quit      
        ...
        % overplot existing leg, but only if it exists (it should as otherwise the togglebutton wouldn't have been down)
          if v.MouseTrack.LegRH.Centroid{MouseIndex}(N,1) > 0
            ...
            text(v.MouseTrack.LegRH.Centroid{MouseIndex}(N,2)+17,v.MouseTrack.LegRH.Centroid{MouseIndex}(N,1)-17,'RH', 'Color', p.colorHind/2, 'LineWidth', 2, 'FontSize', p.FootFontSize); 
          end
        % Turn footprint off till as long as the current footprint is placed (i.e. until there is a gap).
          i = N;
        while i ~= -1
